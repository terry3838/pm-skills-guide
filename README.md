...
flowchart TB
    subgraph 전략["🎯 전략 수립"]
        PS[pm-product-strategy\n제품 전략 12개]
        PD[pm-product-discovery\n제품 발견 13개]
    end
    subgraph 시장["🔍 시장 이해"]
        MR[pm-market-research\n시장 조사 7개]
        GTM[pm-go-to-market\n시장 진입 6개]
    end
    subgraph 실행["⚡ 실행"]
        EX[pm-execution\n실행 15개]
        MG[pm-marketing-growth\n마케팅/성장 5개]
    end
    subgraph 분석["📊 분석/지원"]
        DA[pm-data-analytics\n데이터 분석 3개]
        TK[pm-toolkit\n도구함 4개]
    end
...
```mermaid
flowchart LR
    subgraph 초급["📗 초급 경로 (1-2주)"]
        direction TB
        S1[product-strategy] --> S2[market-segments]
        S2 --> S3[beachhead-segment]
        S3 --> S4[competitor-analysis]
        S4 --> S5[value-proposition]
        S5 --> S6[north-star-metric]
        S6 --> S7[pricing-strategy]
        S7 --> S8[customer-journey-map]
        S8 --> S9[brainstorm-experiments]
    end
    subgraph 중급["📘 중급 경로 (3-4주)"]
        direction TB
        M1[prioritization-frameworks] --> M2[brainstorm-okrs]
        M2 --> M3[gtm-strategy]
        M3 --> M4[pre-mortem]
        M4 --> M5[outcome-roadmap]
        M5 --> M6[create-prd]
    end
    subgraph 고급["📕 고급 경로 (5주+)"]
        direction TB
        A1[growth-loops] --> A2[cohort-analysis]
        A2 --> A3[ab-test-analysis]
        A3 --> A4[lean-canvas]
        A4 --> A5[business-model]
        A5 --> A6[ansoff-matrix]
    end
    초급 --> 중급
    중급 --> 고급
