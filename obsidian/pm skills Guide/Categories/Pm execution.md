...
> [!info]
> source: `categories/pm-execution.md`  
> role: `category`
...
**Step 1 — 명령어 실행**
```
/pm-execution:sprint-plan SprintX Sprint [REDACTED_PHONE] ~ [REDACTED_PHONE])
```
...
```markdown
## Sprint 5 계획 ([REDACTED_PHONE] ~ [REDACTED_PHONE])
...
**Step 1 — 명령어 실행**
```
/pm-execution:retro SprintX Sprint 4 회고 ([REDACTED_PHONE] ~ [REDACTED_PHONE])
```
...
**날짜**: [REDACTED_PHONE] | **형식**: Start / Stop / Continue
...
```markdown
# SprintX v0.5.0 업데이트 노트 ([REDACTED_PHONE])
...
**Step 1 — 명령어 실행**
```
/pm-execution:pre-mortem SprintX AI 실행 엔진 출시 (목표: [REDACTED_PHONE])
```
...
```markdown
## Pre-Mortem: SprintX AI 실행 엔진 출시 ([REDACTED_PHONE])
...
```markdown
## SprintX 이해관계자 맵 ([REDACTED_PHONE])
...
**Step 1 — 명령어 실행**
```
/pm-execution:summarize-meeting [REDACTED_PHONE] SprintX 베타 테스터 인터뷰 #1
```
...
[베타 사용자 A, 베타 테스터] 처음엔 Goal이 뭔지 몰라서 좀 헤맸어요.
예시가 있으면 좋겠다 싶었어요.

[PO] AI 에이전트에게 Task를 위임하는 기능은 어떻게 쓰셨나요?

[베타 사용자 B] 아, 그게 어디 있는지 한참 찾았어요. 버튼이 너무 작았어요.
그리고 위임하고 나서 결과를 어디서 보는지도 몰랐고요.

[PO] 앞으로도 계속 쓰실 것 같으세요?

[베타 사용자 B] 네, 쓸 것 같아요. 근데 지금은 좀 불편해서...
...
**날짜**: [REDACTED_PHONE]:00~15:00
**참석자**: 제품 소유자(PO), 베타 테스터(스타트업 1인 PO 역할 체험자)
...
t-001,온보딩 플로우 1단계 개선,completed,[REDACTED_PHONE]:00:00,[REDACTED_PHONE]:23:00,claude-3-5-sonnet,4980,0.96,
t-002,Goal 생성 UX 개선,completed,[REDACTED_PHONE]:00:00,[REDACTED_PHONE]:45:00,claude-3-5-sonnet,6300,0.91,
t-031,Worker daemon 구현 - Phase 1,failed,[REDACTED_PHONE]:00:00,[REDACTED_PHONE]:30:00,claude-3-5-sonnet,5400,0.23,TypeError: Cannot read property webhookUrl of undefined
```
...
```markdown
## SprintX 백로그 우선순위 재정렬 ([REDACTED_PHONE])
