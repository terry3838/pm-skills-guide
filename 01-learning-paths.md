...
| **다음 스킬로 전달되는 것** | 타겟 고객 정의 -> `market-segments` 입력, 경쟁 환경 초안 -> `competitor-analysis` 입력 |
| **예상 소요 시간** | 1~2시간 |
...
| **다음 스킬로 전달되는 것** | 세그먼트 목록 -> `beachhead-segment`에서 1차 타겟 선택의 후보군 |
| **예상 소요 시간** | 1시간 |
...
| **다음 스킬로 전달되는 것** | 1차 타겟 정의 -> `competitor-analysis`에서 "이 세그먼트의 경쟁자"로 범위 한정, `value-proposition`에서 "이 세그먼트의 고통"에 집중 |
| **예상 소요 시간** | 45분 |
...
| **다음 스킬로 전달되는 것** | 경쟁자 대비 차별화 기회 -> `value-proposition`에서 "왜 우리인가"의 근거 |
| **예상 소요 시간** | 1~2시간 |
...
| **다음 스킬로 전달되는 것** | 핵심 가치 제안 -> `north-star-metric`에서 "어떤 가치 전달을 측정할 것인가"의 기준 |
| **예상 소요 시간** | 1시간 |
...
| **다음 스킬로 전달되는 것** | 핵심 지표 -> `pricing-strategy`에서 "어떤 기능이 핵심 가치 전달에 필수인가"의 판단 기준 |
| **예상 소요 시간** | 45분 |
...
| **다음 스킬로 전달되는 것** | 가격 확정 -> `customer-journey-map`에서 "수익화 단계"의 구체적 내용 |
| **예상 소요 시간** | 1시간 |
...
| **다음 스킬로 전달되는 것** | 끊김 지점 + 가설 -> `brainstorm-experiments`에서 실험 대상 |
| **예상 소요 시간** | 1~1.5시간 |
...
| **다음 스킬로 전달되는 것** | 실험 백로그 -> 중급 경로 `prioritization-frameworks`에서 전략적 우선순위 정렬의 입력 |
| **예상 소요 시간** | 1시간 |
...
├── Day 2-3: market-segments + beachhead-segment (세그먼트 + 1차 타겟)
└── Day 4-5: competitor-analysis + value-proposition (경쟁 분석 + 가치 제안)

Week 2 (전략 확정)
├── Day 1-2: north-star-metric (핵심 지표)
├── Day 2-3: pricing-strategy (가격 확정)
└── Day 4-5: customer-journey-map + brainstorm-experiments (여정 + 실험 설계)
```
...
```
Week 3 (전략 -> 실행 전환)
├── Day 1-2: prioritization-frameworks (백로그 재정렬)
├── Day 2-3: brainstorm-okrs (OKR 설정)
└── Day 4-5: gtm-strategy (GTM 계획 + 첫 실험 시작)

Week 4 (실행 계획 완성)
├── Day 1-2: pre-mortem (리스크 사전 식별)
├── Day 3-4: outcome-roadmap (결과 로드맵)
└── Day 4-5: create-prd (최우선 기능 PRD 작성 -> 개발 핸드오프)
```
