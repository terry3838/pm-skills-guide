...
```mermaid
flowchart LR
    S1["1️⃣ product-strategy\n제품 전략 캔버스"] --> S2["2️⃣ market-segments\n시장 세그먼트"]
    S2 --> S3["3️⃣ beachhead-segment\nBeachhead 선택"]
    S3 --> S4["4️⃣ competitor-analysis\n경쟁 분석"]
    S4 --> S5["5️⃣ value-proposition\n가치 제안"]
    S5 --> S6["6️⃣ north-star-metric\n노스스타 메트릭"]
    S6 --> S7["7️⃣ pricing-strategy\n가격 전략"]
    S7 --> S8["8️⃣ customer-journey-map\n고객 여정 맵"]
    S8 --> S9["9️⃣ brainstorm-experiments\n실험 설계"]
```
...
```mermaid
flowchart LR
    M1["1️⃣ prioritization-frameworks\n전략 기반 우선순위"] --> M2["2️⃣ brainstorm-okrs\nOKR 브레인스토밍"]
    M2 --> M3["3️⃣ gtm-strategy\nGTM 전략"]
    M3 --> M4["4️⃣ pre-mortem\n사전 부검"]
    M4 --> M5["5️⃣ outcome-roadmap\n결과 로드맵"]
    M5 --> M6["6️⃣ create-prd\nPRD 작성"]
```
...
```mermaid
flowchart LR
    A1["1️⃣ growth-loops\n성장 루프 설계"] --> A2["2️⃣ cohort-analysis\n코호트 분석"]
    A2 --> A3["3️⃣ ab-test-analysis\nA/B 테스트 분석"]
    A3 --> A4["4️⃣ lean-canvas\n린 캔버스"]
    A4 --> A5["5️⃣ business-model\n비즈니스 모델 캔버스"]
    A5 --> A6["6️⃣ ansoff-matrix\n안소프 매트릭스"]
```
